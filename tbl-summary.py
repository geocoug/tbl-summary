...
class DataSummary:
    ...
    def table_data(self):
        cur = self.db_inst.execute(f"""SELECT * FROM "{self.schema}"."{self.table}";""")
...
def clparser():
    ...
    parser.add_argument(
        "-t", "--table", type=str, dest="table", help="Table name to summarize."
    )
    return parser
...
def write_summary(data, ofile):
    ...
    for col in range(len(data.columns())):
        ...
        # Most frequent value
        sheet.cell(row=5, column=col + 2).value = data.most_frequent_value(
            data.columns()[col]
        )[0]
        # Value frequency
        sheet.cell(row=6, column=col + 2).value = data.most_frequent_value(
            data.columns()[col]
        )[1]
        # Column data type
...
if __name__ == "__main__":
    ...
    db_inst = Database(args.host, args.database, args.username)
